 ...
 public:
  ...
  void createTrack(ifstream* midiFile, int trackNumber)
  {
    ...
    if (tmp[0] == 'M' && tmp[1] == 'T' && tmp[2] == 'r' && tmp[3] == 'k')
    {
      ...
      cout << "Here2" << endl;
      cout << (int)tmp[0] << " " << (int)tmp[1] << " " << (int)tmp[2] << " " << (int)tmp[3] << endl;
      if (tmp[3] < 0)
        tmp[3] = abs(tmp[3]);
      long trackLength   = charArrayToInt(tmp, TRACK_CHUNK_LENGTH_BYTE_LENGTH);
      int  runningLength = 0;
